 ...
 */
void player_balbum_onenter()
{
    /* restore window background */
    player.mwin->bgcolor.value = MAIN_WINDOW_BGCOLOR;

    gs_win_clear(player.mwin);
    gs_wlist_draw(player.widget.alblist);
 ...
 * process message
 */
int player_balbum_procmsg(struct player_msg_t *msg)
{
    uint32 button;
    switch (msg->id)
    {
...
#define BIG_SCROLL    2
            ...
            {
                gs_wlist_set_cur(player.widget.alblist, GS_WLIST_SET_CUR_PREV,
                ...
                gs_wlist_set_cur(player.widget.alblist, GS_WLIST_SET_CUR_NEXT,
                        BUTTON_PRESSED(button, BUTTON_DOWN) ? 1 : BIG_SCROLL);

                gs_wlist_draw(player.widget.alblist);
                gs_win_refresh(player.mwin);
            } else if (BUTTON_PRESSED(button, BUTTON_BACK)) {
                return PLAYER_MODE_ARTIST_BROWSER;
            } else if (BUTTON_PRESSED(button, BUTTON_ENTER)) {
                char *artist;
                char *album;

                artist = GS_WLIST_GET_CUR_ST1(player.widget.artlist);
                album  = GS_WLIST_GET_CUR_ST1(player.widget.alblist);
                if (artist && album)
                {
                    DPRINT("<album \">s<\" selected>n", album);
                    player_large_msg(PLAYER_INFOMSG_PLEASE_WAIT);

                    if (player_scan_tracks(artist, album))
                    {
                        player_get_cover(artist, album);
                        return PLAYER_MODE_TRACK_BROWSER;
                    }

                    player_large_msg("No tracks");
                    os_wait_ms(1000);
                    player_balbum_onenter();
                }
            }
