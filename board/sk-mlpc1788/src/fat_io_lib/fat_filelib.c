...
int fl_attach_media(fn_diskio_read rd, fn_diskio_write wr)
{
    ...
    {
        dprint("s4d*)*!n", "FAT_FS: Error could not load FAT details (", res);
        return res;
    }
...
void fl_listdirectory(const char *path)
{
    ...
    if (fl_opendir(path, &dirstat))
    {
        ...
        while (fl_readdir(&dirstat, &dirent) == 0)
        {
            if (dirent.is_dir)
            {
                dprint("4dtgss4x n",    ++filenumber, 10, dirent.filename, " <DIR>  cl ", dirent.cluster);
            }
            else
            {
                dprint("4dtgss4xs4d n", ++filenumber, 10, dirent.filename, " <FILE> cl ", dirent.cluster, ", LEN ", dirent.size);
//                dprint("4dss s4ds 4x n", ++filenumber, " - ", dirent.filename, " [", dirent.size, " bytes]", dirent.cluster);
            }
        }
